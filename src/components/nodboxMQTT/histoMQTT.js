...
module.exports = {  // cached singleton instance
    ...
    if (userID && connectionProps) {
      ...
      MQTT.createClient(this.conProps)
        .then((client) => {
          ...
          // alert(`MQTT.createtClient error: ${error}`)
  ...
  onMessageArrived(message) {
    if (message) {
      ...
      //   AsyncStorage.getItem('histo_data').then((data) => {
      //     if(data === null){
      //     data= newdata
      //       AsyncStorage.setItem('histo_data',data)
      //     }else{
      //       data= newdata
      //       AsyncStorage.removeItem('histo_data').then(
