...
export default class ListeStudents extends Component {
    constructor (props){
        super(props)
    ...
    async refresh(){
        ...
        let studentEvalues = await AsyncStorage.getItem('gcam_student_evalue')
    ...
    async getAll(){
        ...
        try {
            let indextotal = await AsyncStorage.getItem('gcam_totalStudent')
            console.log("d"+indextotal);
            if (parseInt(indextotal) === 1) {
                console.log("rrr");
                const first_name = await AsyncStorage.getItem('@gcam_student_first_name'+indextotal)
                const last_name = await AsyncStorage.getItem('@gcam_student_last_name'+indextotal)
                const id = await AsyncStorage.getItem('@gcam_student_id'+indextotal)
                const email = await AsyncStorage.getItem('@gcam_student_email'+indextotal)
                let obj  ={
            ...
            for (let index = 1; index < parseInt (indextotal)+1; index++) {
                const first_name = await AsyncStorage.getItem('@gcam_student_first_name'+index.toString())
                const last_name = await AsyncStorage.getItem('@gcam_student_last_name'+index.toString())
                const id = await AsyncStorage.getItem('@gcam_student_id'+index.toString())
                const email = await AsyncStorage.getItem('@gcam_student_email'+index.toString())
                let obj  ={
                    "name" : first_name +" "+last_name,
                    "id" : id,
                    "email":email
   ...
   storeStudentId(index){
        let obj =[]
        obj.push(index)
        AsyncStorage.getItem('gcam_student_evalue' ).then( (data) => {
            if(data === null){
            data=JSON.stringify(obj)
            AsyncStorage.setItem('gcam_student_evalue',data)
            ...
            AsyncStorage.removeItem('gcam_student_evalue').then(
             
            AsyncStorage.setItem('gcam_student_evalue',JSON.stringify(datas))
            )
            }
                        ...
                        <TouchableOpacity style={{height:150, 
                            ...
                            onPress = {()=>
                                {   this.storeStudentId(this.state.liste.findIndex(it=>it === item)+1)
                                    Actions.accueilGCAM({asyncId: this.state.liste.findIndex(it=>it === item)+1,
