...
export default class ListeStudentSU extends Component {
    constructor (props){
        super(props)
    ...
    async refresh(){
        ...
        let studentEvalues = await AsyncStorage.getItem('su_student_evalue')
    ...
    async getAll(){
        ...
        try {
            let indextotal = await AsyncStorage.getItem('su_totalStudent')
            console.log("d"+indextotal);
            if (parseInt(indextotal) === 1) {
                console.log("rrr");
                const first_name = await AsyncStorage.getItem('@su_student_first_name'+indextotal)
                const last_name = await AsyncStorage.getItem('@su_student_last_name'+indextotal)
                const id = await AsyncStorage.getItem('@su_student_id'+indextotal)
                const email = await AsyncStorage.getItem('@su_student_email'+indextotal)
                let obj  ={
            ...
            for (let index = 1; index < parseInt (indextotal)+1; index++) {
                const first_name = await AsyncStorage.getItem('@su_student_first_name'+index.toString())
                const last_name = await AsyncStorage.getItem('@su_student_last_name'+index.toString())
                const id = await AsyncStorage.getItem('@su_student_id'+index.toString())
                const email = await AsyncStorage.getItem('@su_student_email'+index.toString())
                let obj  ={
                    "name" : first_name +" "+last_name,
                    "id" : id,
                    "email":email
    ...
    checkifAllEvalue(){
        ...
        if (studentEvalue !== null) {
                ...
                console.log(jsonStudentE);
                console.log(this.state.liste);
            if (jsonStudentE.length === this.state.liste.length) {
    ...
    async storeStudentId(index){
        let obj =[]
        obj.push(index)
        AsyncStorage.getItem('su_student_evalue' ).then( (data) => {
            if(data === null){
            data=JSON.stringify(obj)
            AsyncStorage.setItem('su_student_evalue',data)
            ...
            AsyncStorage.removeItem('su_student_evalue').then(
             
            AsyncStorage.setItem('su_student_evalue',JSON.stringify(datas))
            )
            }
                        ...
                        <TouchableOpacity style={{height:150, 
                            ...
                            onPress = {()=>
                                {   this.storeStudentId(this.state.liste.findIndex(it=>it === item)+1)
                                    Actions.accueilUrg({asyncId: this.state.liste.findIndex(it=>it === item)+1,
                                         stage:this.props.stage,stageName : this.props.stageName,typeF:this.props.typeF
                ...
                <TouchableOpacity 
                    ...
                    onPress = {()=> {
                        ...
                        Actions.popTo("homP", {stage:this.props.stage,stageName : this.props.stageName,})
                            ...
                            >
                <Text style={{color:'#fff',fontSize:20}}>CHANGER D'OUTILS</Text>
                </TouchableOpacity >
                </View>:(null)
                } */}
