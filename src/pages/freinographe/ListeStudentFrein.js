...
export default class ListeStudentF extends Component {
    constructor (props){
        super(props)
    ...
    async refresh(){
        ...
        let studentEvalues = await AsyncStorage.getItem('f_student_evalue')
    ...
    async getAll(){
        ...
        try {
            let indextotal = await AsyncStorage.getItem('f_totalStudent')
            console.log("d"+indextotal);
            if (parseInt(indextotal) === 1) {
                console.log("rrr");
                const first_name = await AsyncStorage.getItem('@f_student_first_name'+indextotal)
                const last_name = await AsyncStorage.getItem('@f_student_last_name'+indextotal)
                const id = await AsyncStorage.getItem('@f_student_id'+indextotal)
                const email = await AsyncStorage.getItem('@f_student_email'+indextotal)
                let obj  ={
            ...
            for (let index = 1; index < parseInt (indextotal)+1; index++) {
                const first_name = await AsyncStorage.getItem('@f_student_first_name'+index.toString())
                const last_name = await AsyncStorage.getItem('@f_student_last_name'+index.toString())
                const id = await AsyncStorage.getItem('@f_student_id'+index.toString())
                const email = await AsyncStorage.getItem('@f_student_email'+index.toString())
    ...
    async storeStudentId(index){
        let obj =[]
        obj.push(index)
        AsyncStorage.getItem('f_student_evalue' ).then( (data) => {
            if(data === null){
            data=JSON.stringify(obj)
            AsyncStorage.setItem('f_student_evalue',data)
            ...
            AsyncStorage.removeItem('f_student_evalue').then(
             
            AsyncStorage.setItem('f_student_evalue',JSON.stringify(datas))
            )
            }
                        ...
                        <TouchableOpacity style={{height:150, 
                            ...
                            onPress = {()=>
                                {   this.storeStudentId(this.state.liste.findIndex(it=>it === item)+1)
                                    Actions.accueilFrein({asyncId: this.state.liste.findIndex(it=>it === item)+1,
            ...
            <View style={styles.container}>
               <View style={styles.headerContainer}>
                 ...
                 <Image
                    ...
                    />
                      <Text style={styles.headerTitle}>FREINOGRAPHE</Text>
                      </View>
