...
import {Icon} from 'react-native-elements';
  // import PreparerScreen from './PreparerScreen'
  // import AnticiperScreen from './AnticiperScreen'
  // import VoirScreen from './VoirScreen'
  // import PrevoirScreen from './PrevoirScreen'
...
class AccueilP extends React.Component{
  constructor(props) {
    super(props);
    this.state = {
		expandedPrep1: false,
  ...
  current=()=>{
    ...
    //   type:'octicon'
    // }
    // AsyncStorage.getItem('time_dataP').then((data) => {
    //   if(data === null){
    //     data= JSON.stringify(time)
    //     AsyncStorage.setItem('time_dataP',data)
    //   }else{
    //   data=data+','+JSON.stringify(time)
    //   AsyncStorage.removeItem('time_dataP').then(
    ...
    AsyncStorage.getItem( 'dataP' ).then( (data) => {
      if(data === null){
        data= JSON.stringify(obj)
        AsyncStorage.setItem('dataP',data)
      }else{
      data=data+','+JSON.stringify(obj)
      AsyncStorage.removeItem('dataP').then(
    ...
    let objT={
      ...
      AsyncStorage.getItem( 'dataFT1' ).then( (data) => {
        if(data === null){
          data= JSON.stringify(objT)
          AsyncStorage.setItem('dataFT1',data)
        }else{
        data=data+','+JSON.stringify(objT)
        AsyncStorage.removeItem('dataFT1').then(
